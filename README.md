# Hi there 👋 I'm Bhargav Kagtada!

Welcome to my GitHub profile! I'm passionate about open-source development, web technologies, and continuous learning.

## 🚀 About Me

- 🌱 I’m currently learning **React**, **Node.js**, and **cloud technologies**
- 👯 I’m looking to collaborate on _interesting open-source projects_
- 💬 Ask me about _web development, JavaScript, or GitHub workflows_
- 📫 How to reach me: [LinkedIn](https://www.linkedin.com/in/bhargavkagtada/) | [Email](mailto:your-email@example.com)

## 📌 Featured Projects

Here are a few projects I'm proud to share:

| Project | Description | Tech Stack | Link |
| ------- | ----------- | ---------- | ---- |
| **Project 1** | A web app for tracking tasks and productivity | React, Node.js, MongoDB | [GitHub Repo](https://github.com/bhargavkagtada/project1) |
| **Project 2** | API for real-time data aggregation | Express, Python, REST | [GitHub Repo](https://github.com/bhargavkagtada/project2) |
| **Project 3** | Automation scripts and tools | Bash, Python | [GitHub Repo](https://github.com/bhargavkagtada/project3) |

## 🛠️ Skills & Tools

![JavaScript](https://img.shields.io/badge/-JavaScript-black?style=flat-square&logo=javascript)
![React](https://img.shields.io/badge/-React-blue?style=flat-square&logo=react)
![Node.js](https://img.shields.io/badge/-Node.js-green?style=flat-square&logo=node.js)
![Python](https://img.shields.io/badge/-Python-yellow?style=flat-square&logo=python)
![MongoDB](https://img.shields.io/badge/-MongoDB-green?style=flat-square&logo=mongodb)
![Azure](https://img.shields.io/badge/-Azure-0078D4?style=flat-square&logo=microsoft-azure)
![Cosmos DB](https://img.shields.io/badge/-CosmosDB-0089D6?style=flat-square&logo=azure-cosmos-db&logoColor=white)
![Microsoft Fabric](https://img.shields.io/badge/-Microsoft%20Fabric-3B91F7?style=flat-square)
![Azure OpenAI](https://img.shields.io/badge/-Azure%20OpenAI-0078D4?style=flat-square&logo=openai&logoColor=white)
![AI Search](https://img.shields.io/badge/-AI%20Search-03C7C9?style=flat-square)

## 📈 GitHub Stats
![Bhargav's GitHub stats](https://github-readme-stats.vercel.app/api?username=bhargavkagtada&show_icons=true&theme=dracula)

---

> _“Code is like humor. When you have to explain it, it’s bad.”_ – Cory House
